...
//    }
    ...
    SparkConf sparkConf = new SparkConf().setAppName("JavaPageRank").setMaster("local[1]");
    ...
    //     URL         neighbor URL
    //     ...
    JavaRDD<String> lines = ctx.textFile("/Users/hutianqi/Downloads/test/test/pageRankTest", 1);
    List<String> strLines = lines.collect();
...
//    }
    ...
    List strLinks = links.collect();
    for(Object str : strLinks){
    	System.out.println("hutianqi");
    ...
    for (int current = 0; current < 1; current++) {
      ...
      List contribs1 = links.join(ranks).values().collect();
      for(Object obj : contribs1){
